...
export function wallBuildSteps(parent, room, wall, connectedDoorIds) {
...
export function buildWall(parent, room, wall, connectedDoorIds) {
  for (const step of wallBuildSteps(parent, room, wall, connectedDoorIds)) step();
}

export function skylightBuildSteps(parent, template) {
  ...
  for (let i = 1; i < 5; i += 1) steps.push(() => box(parent, { position: `${-openingWidth / 2 + i * openingWidth / 5} ${template.height} 0`, width: .026, height: .04, depth: openingDepth, color: '#a69d92', shadow: false }));
  for (let i = 1; i < 4; i += 1) steps.push(() => box(parent, { position: `0 ${template.height} ${-openingDepth / 2 + i * openingDepth / 4}`, width: openingWidth, height: .04, depth: .026, color: '#a69d92', shadow: false }));
  return steps;
}

export function buildSkylight(parent, template) {
  for (const step of skylightBuildSteps(parent, template)) step();
}

export function trackLightBuildSteps(parent, template) {
  const xTracks = [-template.width * .34, template.width * .34];
  const steps = [];
  for (const x of xTracks) {
    steps.push(() => box(parent, { position: `${x} ${template.height - .55} 0`, width: .025, height: .025, depth: template.depth * .74, color: '#3a3530', shadow: false }));
    for (let index = -3; index <= 3; index += 1) {
      const z = index * template.depth * .1;
      steps.push(() => entity('a-cylinder', { position: `${x} ${template.height - .67} ${z}`, radius: .055, height: .18, rotation: '90 0 0', material: 'color: #4b4540; emissive: #e8d7bd; emissiveIntensity: .2; roughness: .42' }, parent));
      steps.push(() => entity('a-entity', { position: `${x} ${template.height - .73} ${z}`, light: 'type: point; color: #ffe2bd; intensity: .19; distance: 5.5; decay: 2' }, parent));
    }
  }
  return steps;
}
...
export function buildTrackLights(parent, template) {
  for (const step of trackLightBuildSteps(parent, template)) step();
}
